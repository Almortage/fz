...
from SEMOMUSIC.Data import get_data
from googletrans import Translator
from SEMOMUSIC.Data import (get_call, get_app, get_userbot, get_group, get_channel, must_join)
...
async def gen_thumb(videoid, photo):
    ...
    try:
        ...
        SEMOMUSICv = Image.open(f"{photo}")
        image1 = changeImageSize(1280, 720, youtube)
        image2 = image1.convert("RGBA")
        background = image2.filter(filter=ImageFilter.BoxBlur(5))
        enhancer = ImageEnhance.Brightness(background)
        background = enhancer.enhance(0.6)
        Xcenter = SEMOMUSICv.width / 2
        Ycenter = SEMOMUSICv.height / 2
        x1 = Xcenter - 250
        y1 = Ycenter - 250
        x2 = Xcenter + 250
        y2 = Ycenter + 250
        logo = SEMOMUSICv.crop((x1, y1, x2, y2))
