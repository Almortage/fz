...
from SEMOMUSIC.info import (db, add, is_served_call, add_active_video_chat, add_served_call, add_active_chat, gen_thumb, download, remove_active, joinch)
from SEMOMUSIC.Data import (get_logger, get_userbot, get_call, get_logger_mode, get_group, get_channel)
import asyncio
...
async def play(client: Client, message):
  ...
  if not message.reply_to_message:
     ...
     if await is_served_call(client, message.chat.id):
         ...
         requester = chatname if SEMOMUSIC.views else f"[{message.from_user.first_name}](tg://user?id={message.from_user.id})"
